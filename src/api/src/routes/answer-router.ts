...
answerRouter.get("/", checkJwt, loadUser, requireAdminOrOwner, async (req: Request, res: Response) => {
  let list = await answerService.getAll();

  res.json({ data: list });
});

answerRouter.put("/:id", checkJwt, loadUser, requireAdminOrOwner, async (req: Request, res: Response) => {
  let { id } = req.params;
